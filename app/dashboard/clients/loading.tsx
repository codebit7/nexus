    ...
    <div className="flex flex-col h-full bg-[var(--bg-page)] animate-pulse">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 sm:p-6">
          ...
          {/* Stats cards — grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 (4 cards) */}
          ...
          <div className="rounded-lg border border-[var(--border-subtle)] overflow-hidden">
            ...
            <div className="flex items-center gap-4 bg-[var(--bg-sidebar)] border-b border-[var(--border-subtle)]
              ...
              <div className={`h-3 w-12 ${BAR}`} />
              <div className={`hidden md:block h-3 w-24 ${BAR}`} />
              <div className={`hidden lg:block h-3 w-28 ${BAR}`} />
              <div className="w-8" />
            </div>
            {/* Rows */}
            {[0, 1, 2, 3, 4].map((i) => (
              <div key={i}
                ...
                {/* Monthly rate — hidden md:table-cell */}
                <div className={`hidden md:block h-4 w-20 ${BAR}`} />
                {/* Active projects — hidden lg:table-cell */}
                <div className={`hidden lg:block h-4 w-20 ${BAR}`} />
                {/* Chevron */}

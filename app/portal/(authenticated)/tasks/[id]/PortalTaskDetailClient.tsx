...
function CommentItem({ comment, clientId }: { comment: Comment; clientId: string }) {
  ...
  const authorName = isClient
    ? "You"
    : ((comment as Record<string, unknown>).author_name as string | null) ?? "Team Member";
              ...
              <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-sidebar)] overflow-hidden sticky top-6">
                <div className="p-4 border-b border-[var(--border-subtle)]">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h2 className="text-[13px] font-medium text-[var(--text-primary)]">All Tasks</h2>
                  </div>
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center justify-between gap-2 mb-1.5">
                            ...
                            <span
                              className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-medium shrink-0"
                          ...
                          <div className="flex items-center justify-between gap-2 text-[11px]">
                            <div className="flex items-center gap-2">
                              <span className="text-[var(--text-faint)]">{priorityConfig.label}</span>
                              {t.due_date && (
            ...
            <div className="flex-1 min-w-0">
              <div className="space-y-6">
                    ...
                    <div className="flex items-start justify-between gap-4 mb-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <StatusBadge status={(task.status as TaskStatus) ?? "todo"} />
                          <PriorityBadge priority={(task.priority as TaskPriority) ?? "normal"} />
                          {isOverdue && (
                ...
                <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-sidebar)] p-6">
                  ...
                  {files.length === 0 ? (
                    <div className="rounded-lg border border-dashed border-[var(--border-default)] bg-[var(--bg-input)] px-6 py-8 text-center">
                      <FileText className="mx-auto h-8 w-8 text-[var(--text-faint)] mb-2" />
                      <p className="text-sm text-[var(--text-faint)]">No attachments yet</p>
                      <p className="text-xs text-[var(--text-disabled)] mt-1">Files shared by the team will appear here</p>
                    </div>
                  ) : (
